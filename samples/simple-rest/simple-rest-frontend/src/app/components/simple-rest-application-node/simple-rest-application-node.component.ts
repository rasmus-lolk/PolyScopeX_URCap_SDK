...
export class SimpleRestApplicationNodeComponent implements ApplicationPresenter, OnChanges {
    ...
    getExample = async (): Promise<void> => {
        const data = await this.fetchData(`${location.protocol}//${this.backendUrl}/get-example`);
        console.log(data);
    };

    postExample = async (): Promise<void> => {
        const data = await this.fetchData(`${location.protocol}//${this.backendUrl}/post-example`, {
