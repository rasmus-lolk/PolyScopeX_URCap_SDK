...
export class RosHelper {
    ...
    public static async subscribeToStatusTopic(ros2Client: Ros2Client): Promise<Observable<{ data: string; }>> {
        const topicPath = await ros2Client.getTopicPath(true,ROS_STATUS,URCAP_ROS_NAMESPACE);
    ...
    private static async callService(
        ros2Client: Ros2Client,
        serviceName: string,
        ...
        qos.depth = 1;
        const servicePath = await ros2Client.getServicePath(true, serviceName, URCAP_ROS_NAMESPACE);
