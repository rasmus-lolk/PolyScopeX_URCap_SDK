...
export class SimpleWebsocketApplicationNodeComponent implements ApplicationPresenter, OnChanges {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes?.robotSettings) {
            ...
            // Https and Wss are needed only for testing in github codespace
            const protocol = location.protocol === 'https:' ? 'wss' : 'ws';
            this.backendUrl = protocol+'://' + this.applicationAPI.getContainerContributionURL(VENDOR_ID, URCAP_ID, 'simple-websocket-backend', 'websocket-api') + '/';
        }
    }
