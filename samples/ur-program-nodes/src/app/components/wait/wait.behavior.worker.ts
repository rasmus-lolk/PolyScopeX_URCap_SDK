            ...
            case 'signalInput': {
                ...
                if (signalInput?.signalID && signalInput?.sourceID && signalInput.value !== undefined) {
                    ...
                    if (typeof signalInput.value === 'boolean') {
                        ...
                        return `${signalName}==${signalInput.value}`;
                    } else {
    ...
    generateCodeBeforeChildren: async (node: SampleWaitNode): Promise<ScriptBuilder> => {
        ...
        switch (node.parameters.type) {
            case 'signalInput': {
                ...
                let right;
                switch (typeof signalInput.value) {
                    case 'boolean':
                    ...
                    // if the value is in Voltages, we need no conversion
                        right = signalInput.value.unit === 'mA' ? convertValue(signalInput.value, 'A').value : signalInput.value.value;
                        whileExpression = `${left} ${signalInput?.analogOperator} ${right}`;
                        break;
                }
